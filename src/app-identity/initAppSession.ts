 ...
 *
 * The per-collection KAKs themselves are derived where they are consumed
 * (`LocalStore.init` calls `deriveCollectionKeys` per collection); this module
 * only surfaces the identity/zcap side the auth flow needs.
 */
import { deriveIdentity, type IdentityAgents } from '@/app-identity/agents'
