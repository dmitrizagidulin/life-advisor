/**
 * The last-write-wins tiebreak for a mutable head document. Ported from
 * freewallet-mobile's `app/lib/sync/lww.ts`. Pure -- no React, no storage
 * imports -- so both replicas run this identical rule against the same two
 * payloads and converge on the same winner with no coordination.
 *
 ...
 * the encrypted payload (never the envelope-level checkpoint `updatedAt`).
 */
