...
describe('createActionItem defaults', () => {
  it('applies the property defaults', () => {
    const item = createActionItem({ name: 'x', writerId: D }, NOW, 'id-1')
  ...
  it('lets a provided false done through', () => {
    expect(createActionItem({ name: 'x', writerId: D, done: false }).done).toBe(false)
  })
})

describe('createProject defaults', () => {
  it('idea status, null timestamps, empty goalIds, admin area', () => {
    const p = createProject({ name: 'p', writerId: D }, NOW)
  ...
  it('created with completed status stamps completedAt and clears canceledAt', () => {
    const p = createProject({ name: 'p', status: 'completed', writerId: D }, NOW)
  ...
  it('created with canceled status stamps canceledAt and clears completedAt', () => {
    const p = createProject({ name: 'p', status: 'canceled', writerId: D }, NOW)
  ...
  it('created with active status leaves both timestamps null', () => {
    const p = createProject({ name: 'p', status: 'active', writerId: D }, NOW)
...
describe('createGoal defaults', () => {
  it('active true, accomplished false', () => {
    const g = createGoal({ name: 'g', writerId: D })
...
describe('createQuestion defaults', () => {
  it('answered false, answeredAt null', () => {
    const q = createQuestion({ name: 'q', writerId: D })
...
describe('createAnswer', () => {
  it('is always question-parented', () => {
    const a = createAnswer({ writerId: D, parentKey: 'q1' })
...
describe('createWebLink / createThought day default', () => {
  it('web link with no parent defaults to today', () => {
    const l = createWebLink({ url: 'https://x.com', writerId: D }, NOW, 'id', TODAY)
  ...
  it("web link with 'today' sentinel maps to today's date", () => {
    const l = createWebLink(
      { url: 'https://x.com', writerId: D, parentType: 'day', parentKey: 'today' },
      NOW,
      'id',
      TODAY
    )
  ...
  it('web link keeps an explicit real parent', () => {
    const l = createWebLink(
      { url: 'https://x.com', writerId: D, parentType: 'project', parentKey: 'p1' },
      NOW,
      'id',
      TODAY
    )
  ...
  it('thought defaults to today', () => {
    const t = createThought({ name: 'idea', writerId: D }, NOW, 'id', TODAY)
...
describe('createCurrentFocus', () => {
  it('has the fixed singleton id', () => {
    const f = createCurrentFocus({ focusType: 'day', focusKey: 'today', writerId: D })
