 ...
 * every replica compares that same master against its own local edit, and
 * `remotePayloadWins` is a total order over `(updatedAt, deviceId)`, so the
...
export function makeLwwConflictHandler(
  ...
  return {
    ...
    async resolve({
      ...
      if (remote && local) {
        return remotePayloadWins(remote, local)
          ? realMasterState
          : newDocumentState
      }
