 ...
 * than the copied port's `GET` ETag header.
 *
 ...
 * `Access-Control-Expose-Headers: etag`. The was-teaching-server does not (and is
 * read-only here), so cross-origin `get()` reports `version: 0`, which makes the
 * loser of a push race re-push forever with a stale `If-Match` and never
 * converge. The `changes` feed carries `version` (and `data`, `_deleted`, ...) in
 * the JSON BODY, which CORS never strips, so resolving the master from the feed
 * is both correct and origin-independent. Normal pull/push are unaffected (they
 * already read `version` from the feed body), so only `get` is overridden.
 */
import type {
  Json,
  MasterState,
