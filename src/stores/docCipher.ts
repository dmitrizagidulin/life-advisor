/**
 * The per-collection EDV document cipher: the local encrypt/decrypt seam.
 * Adapted from freewallet's `src/stores/edvDocCipher.ts` and freewallet-mobile's
 * `app/lib/sync/docCipher.ts`. It wraps the same `@interop/was-client` EDV codec
 * the remote WAS handles use, pointed at the local replica: `encrypt` turns a
 * JSON document into its stored EDV envelope (`{ id, sequence, jwe }`) minting a
 * stable random resource id (`idDerivation: 'random'`), and `encryptUpdate`
 * re-encrypts a mutable head document under its EXISTING id, advancing the
 * envelope `sequence` from the prior envelope. `decrypt` reverses it. The sync
 * layer moves the envelope verbatim; it never touches these keys.
 */
import type {
  IKeyAgreementKey,
  IKeyResolver
