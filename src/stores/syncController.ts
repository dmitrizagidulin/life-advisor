 ...
 * Adapted from freewallet's `syncController.ts` for the delegated RP model.
 *
 ...
 * caller wires to re-decrypt and patch the zustand store per-doc. Subscribing to
 * the collection
 * stream (rather than the replication `received$`) is deliberate: it also fires
 * for documents rewritten by CONFLICT RESOLUTION on the push path (when this
 * replica adopts the remote master under LWW), which `received$` (pull-only)
 * misses -- otherwise the losing side's UI would show its stale local edit until
 * the next unrelated pull. The caller debounces, and a re-hydrate after this
 * replica's own optimistic write is idempotent.
 */
...
class SyncController {
  ...
  async start({
    ...
    try {
      ...
      // live (the received$ subscriptions above then patch the stores).
