 ...
 * Space. Adapted from freewallet's `wasRemoteStore.ts`, stripped to the RP model
 * -- this app NEVER provisions the Space, never derives the spaceId, and never
 * touches the `id` collection or DID publishing. It receives a set of
 * wallet-delegated zcaps, reads the server URL + space id straight out of their
 * `invocationTarget`s ({@link parseGrants}), and holds:
 *
 ...
 *   collection grant;
 * - a best-effort encryption-marker PUT (an open question in P2: whether a
 *   delegated collection-scoped RW zcap authorizes writing the collection
 *   description). It is non-fatal either way -- envelopes replicate into an
 *   unmarked (plaintext) collection just the same.
 */
