 ...
 *    LifeAdvisorKey credential is self-issued by design).
...
import { documentLoader } from '@/app-identity/seedCredential'
import { parseGrants, type ParsedGrants } from '@/stores/grants'
import { earliestExpiry, isExpired } from '@/session/appSession'
import { LA_COLLECTIONS } from '@/app.config'
import { RW_ACTIONS } from '@/auth/loginRequest'
...
export function checkGrants({
  ...
  if (expectedServerUrl !== undefined && parsed.serverUrl !== expectedServerUrl) {
    throw new Error(
  ...
  for (const { id } of LA_COLLECTIONS) {
    ...
    const missing = RW_ACTIONS.filter((action) => !actions.includes(action))
    if (missing.length > 0) {
      throw new Error(
